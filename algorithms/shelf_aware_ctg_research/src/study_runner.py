...
from .direction_grid import DirectionGridResult, build_direction_grid
from .project_inputs import DEFAULT_PROJECT_DIR, PACKAGE_ROOT, StudyInput, build_study_input, load_maptools_project
...
def build_junction_label_map(mask_shape: tuple[int, int], graph_info: Any) -> np.ndarray:
    labels = np.full(mask_shape, -1, dtype=np.int32)
    for node in getattr(graph_info, "nodes", ()):  # graph_info 来自正式 CTG 输出。
        polygon_rc = tuple(getattr(node, "polygon_vertices_rc", None) or ())
        if len(polygon_rc) < 3:
            continue
        points_xy = np.array([[int(round(float(col))), int(round(float(row)))] for row, col in polygon_rc], dtype=np.int32)
...
def run_shelf_aware_baseline_path(
    study_input: StudyInput,
    geometry_result: Any,
    output_dir: Path,
    local_edge_label_map: np.ndarray | None = None,
    ...
    return run_shelf_aware_path_variant(
        study_input=study_input,
        geometry_result=geometry_result,
        output_dir=output_dir,
        variant_dir_name="shelf_aware_baseline",
        overlay_name="09_shelf_aware_baseline_path_overlay.png",
        local_edge_label_map=local_edge_label_map,
        local_junction_label_map=local_junction_label_map,
    )
...
def run_territory_study_area(
    ...
    *,
    ...
    junction_label_map = build_junction_label_map(np.asarray(geometry_result.free_mask).shape, graph_info)
    shelf_baseline = run_shelf_aware_baseline_path(
        study_input,
        geometry_result,
        output_dir,
        local_edge_label_map=expanded.labels,
        local_junction_label_map=junction_label_map,
    )
    ...
    summary["shelf_aware_baseline"] = shelf_baseline
    (output_dir / "summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return output_dir


def run_default_project_study(
...
def run_projects_territory_study(
    ...
    *,
    ...
    for project_dir in project_dirs:
        project_path = Path(project_dir).expanduser().resolve()
        _map_data, annotations = load_maptools_project(project_path)
