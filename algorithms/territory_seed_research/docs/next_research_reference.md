# 下一步研究算法参考：全局语义、方向先验与网格节点质量

本文记录当前阶段对后续研究方向的判断。重点不是继续组合新算法，而是明确 `shelf_aware_guarded` 当前缺什么、`axis prior / territory seed` 能提供什么、以及更值得优先优化的模块。
...
CTG / territory / axis 只提供全局语义、拓扑证据和方向先验
不要让 axis prior 接管局部细节规划
```

原因：

- `shelf_aware_guarded` 对货架、墙边、局部边界细节表现更好。
- 直接使用 axis prior 替代 baseline local direction 后，部分区域路径变长或更不稳定。
...
`axis prior / territory seed` 可以提供的第一类价值，不应该是直接提供 sweep 方向，而是提供全局语义 label。
...
当前 axis prior 的问题之一是：传入 external axis 后，planner 会用它替代 baseline image-gradient local direction。这样会丢失 baseline 的局部细节优势。
